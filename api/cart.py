...
async def add_to_cart(req: CartAddRequest):
    ...
    async with pool.acquire() as conn:
        row = await conn.fetchrow("SELECT name, min_order_qty, order_unit FROM products WHERE product_id = $1", req.product_id)
        if not row:
...
async def cart_summary(customer_id: int):
    ...
    # The CartItem schema expects product_id, name, qty, unit
    return {"items": items}
